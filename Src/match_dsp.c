...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
	{
		...
		switch(GPIO_Pin)
		{
			case External_IN_Pin:							// ?????? ??????
				...
				{
					blink_ext = 0;
					HAL_GPIO_WritePin(Interrupt_OUT_GPIO_Port,Interrupt_OUT_Pin, GPIO_PIN_RESET);		// включение большого света
					temp = GPIOA->MODER;																														//
				...
				else
				{
					if(blink_end)
						...
						HAL_GPIO_WritePin(Interrupt_OUT_GPIO_Port,Interrupt_OUT_Pin, GPIO_PIN_SET);		// выключение большого света
